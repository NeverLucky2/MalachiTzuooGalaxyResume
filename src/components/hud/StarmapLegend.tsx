...
export function StarmapLegend({
  ...
  return (
    <div className="min-w-[182px] rounded-[14px] border border-[#21e6ff]/40 bg-[#0a0a1f]/60 px-3 py-[10px] shadow-[0_0_16px_rgba(33,230,255,.25)] backdrop-blur-md">
      <div className="mx-1 mb-2 font-display text-[10px] uppercase tracking-[2px] opacity-65">
      ...
      {PLANETS.map((p, i) => {
        ...
        return (
          <button
            ...
            className={`flex w-full items-center gap-[9px] rounded-lg px-2 py-[6px] text-left text-xs text-[#e7f6ff] transition-opacity hover:bg-white/[.06] focus-visible:outline focus-visible:outline-2 focus-visible:outline-offset-1 focus-visible:outline-cyan-300 ${
              cur
          ...
          >
            ...
            />
            <span>{p.label}</span>
            <span className="ml-auto font-display text-[9px] opacity-50">
              {`0${i + 1}`}
            </span>
          </button>
        );
